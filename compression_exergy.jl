...
using PyPlot, Roots, CoolProp, DataFrames
# include("CoolProp.jl")
...
for j in 1:5
  ...
  info("pipeline pressure drop is calculated.")
  ...
  for i in 1:n_stage
    ...
    if gas_type == "CO2" || gas_type == "N2" || gas_type == "CH4"
      T_out=CoolProp.PropsSI("T", "SMOLAR", S_in, "P", p_comp[i+1], gas_type)
    else
      ftemp = T -> S_in - CoolProp.PropsSI("SMOLAR", "T", T, "P", p_comp[i+1], gas_type)
      T_out = fzero(ftemp, T_comp+40)
    end
    # H_out=CoolProp.PropsSI("HMOLAR", "SMOLAR", S_in, "P", p_comp[i+1], gas_type)
  ...
  for i in 1:n_stage_well
    ...
    if gas_type == "CO2" || gas_type == "N2" || gas_type == "CH4"
      ...
      ftemp = T -> S_in - CoolProp.PropsSI("SMOLAR", "T", T, "P", p_comp_well[i+1], gas_type)
      T_out = fzero(ftemp, T_comp+40)
    end
    # H_out=CoolProp.PropsSI("HMOLAR", "SMOLAR", S_in, "P", p_comp_well[i+1], gas_type)
